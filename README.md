...
I build and scale backend systems, data pipelines, and platform infrastructure. Currently architecting a next-gen data platform processing 1 TB/day with a roadmap to multi-terabyte scale.
...
- Owning the architecture of a **large-scale data platform** built on Apache Iceberg, Spark, and Trino (5-engineer team)
- Led the development of an **MCP server** that automates developer workflows and enables seamless debugging
- Spearheading AI adoption (Cursor AI, GitHub Copilot) across the engineering org

### Highlights

- Rewrote a legacy Ruby service in **Go** — cut response time by **75%** (120ms → 30ms)
- Built data pipelines handling **$70M+ in daily transactions** with banking integration
...
- Built a Python CLI that cut onboarding workflows by **99%** (hours → seconds)
- Migrated cold storage from **AWS → GCP** with zero downtime
- Redesigned CI/CD pipelines, eliminating downtime risk during disaster recovery
- Mentored 5+ engineers, fast-tracking their onboarding and productivity
